    ...
    <section id="vsl-section" className="py-12 md:py-20 px-4 md:px-6 bg-black">
      <div className="max-w-5xl mx-auto">
        ...
        >
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              {!hasStarted ? (
                <>
                  <motion.div
                    ...
                    className="w-16 h-16 md:w-24 md:h-24 bg-primary rounded-full flex items-center justify-center shadow-2xl shadow-primary/40 cursor-pointer mb-4 md:mb-6"
                  >
                    <Play className="w-7 h-7 md:w-10 md:h-10 text-primary-foreground ml-1 md:ml-2" fill="currentColor" />
                  </motion.div>
                  <p className="text-white text-base md:text-xl font-semibold mb-2 px-4">
                ...
                <>
                  ...
                  <div className="absolute inset-0 p-4 md:p-8 flex flex-col">
                    ...
                    <div className="flex items-center justify-between mb-auto">
                      <span className="bg-primary/20 text-primary px-2 md:px-3 py-0.5 md:py-1 rounded-full text-xs md:text-sm font-medium">
                    ...
                    <div className="flex-1 flex items-center justify-center">
                      <div className="text-center">
                        <motion.div
                          ...
                          className="w-20 h-20 md:w-32 md:h-32 mx-auto mb-4 md:mb-6 rounded-full bg-gradient-to-br from-primary/30 to-primary/10 flex items-center justify-center"
                        >
                          <div className="w-12 h-12 md:w-20 md:h-20 rounded-full bg-primary/20 flex items-center justify-center">
                            {isPlaying ? (
                              ...
                              <Pause className="w-5 h-5 md:w-8 md:h-8 text-primary" />
                            )}
                          </div>
                        </motion.div>
                        <h3 className="text-lg md:text-2xl font-bold text-white mb-2 px-2">
                          {currentClip?.titulo}
                        </h3>
                    ...
                    <div className="flex flex-wrap gap-1.5 md:gap-2 justify-center mt-auto">
                      <span className="bg-zinc-800 text-zinc-300 px-2 md:px-3 py-0.5 md:py-1 rounded-full text-[10px] md:text-xs flex items-center gap-1">
                        <Building className="w-2.5 h-2.5 md:w-3 md:h-3" /> {nichoLabel}
                      </span>
                      <span className="bg-zinc-800 text-zinc-300 px-2 md:px-3 py-0.5 md:py-1 rounded-full text-[10px] md:text-xs flex items-center gap-1">
                        <User className="w-2.5 h-2.5 md:w-3 md:h-3" /> {formData.empresa || "Tu empresa"}
                      </span>
                    </div>
                  </div>
              ...
              {hasFinished && (
                <motion.div
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center z-20 p-4"
                >
                  ...
                  <p className="text-zinc-400 mb-4 md:mb-6 text-sm md:text-base text-center px-4">
                    Ahora completá algunos datos más para tu plan personalizado
                  </p>
                  <button
                    onClick={handleContinue}
                    className="bg-primary text-primary-foreground px-6 md:px-8 py-3 md:py-4 rounded-xl font-bold hover:bg-primary/90 transition-all text-sm md:text-base"
                  >
          ...
          {/* Video controls */}
          <div className="p-3 md:p-4 bg-zinc-900/90 border-t border-zinc-800">
            {/* Progress bar */}
            <div className="mb-3 md:mb-4">
              <Progress value={totalProgress} className="h-1 md:h-1.5" />
            </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 md:gap-4">
                ...
                <button
                  ...
                  className="w-8 h-8 md:w-10 md:h-10 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <SkipForward className="w-4 h-4 md:w-5 md:h-5 text-white" />
                </button>
                <button
                  onClick={() => setIsMuted(!isMuted)}
                  className="w-8 h-8 md:w-10 md:h-10 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center transition-colors hidden sm:flex"
                >
                  {isMuted ? (
              ...
              <div className="flex items-center gap-2">
                <button className="w-8 h-8 md:w-10 md:h-10 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center transition-colors">
        ...
        <motion.div
          ...
          className="mt-6 md:mt-8 grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-2 md:gap-4"
        >
          {playlist.map((clip, index) => (
            <div
              key={clip.id}
              className={`p-2.5 md:p-4 rounded-lg md:rounded-xl border transition-all cursor-pointer ${
            ...
            >
              <div className="flex items-center gap-2 md:gap-3">
                <div
                  className={`w-6 h-6 md:w-8 md:h-8 rounded-full flex items-center justify-center text-[10px] md:text-xs font-bold shrink-0 ${
                    index < currentClipIndex
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs md:text-sm font-medium text-white truncate">{clip.titulo}</p>
                  <p className="text-[10px] md:text-xs text-zinc-500">{clip.duracion}s</p>
                </div>
              </div>
            </div>
          ))}
