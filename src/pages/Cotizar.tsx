...
const CotizarContent = () => {
  const navigate = useNavigate();
  const { formData, updateFormData, loadFromStorage, submitToWebhook, saveToStorage } = useLeadMagnet();
  ...
  useEffect(() => {
    loadFromStorage();
    analytics.pageView("cotizar");
    const cleanup = setupScrollTracking("cotizar");
    return cleanup;
  }, []);
  ...
  // Track form2 start
  useEffect(() => {
    if (!hasTrackedStart.current && formData.situacion) {
  ...
  }, [formData.situacion, specificQuestions]);
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-lg border-b border-zinc-800">
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-3 md:py-4 flex items-center justify-between">
          <a href="/video" className="flex items-center gap-1.5 md:gap-2 text-muted-foreground hover:text-foreground transition-colors">
      ...
      <main className="pt-14 md:pt-20">
        <section className="py-12 md:py-20 px-4 md:px-6 bg-gradient-to-b from-black to-background">
                        ...
                        return (
                          <>
                            ...
                            <div className="flex justify-start pt-4">
                              <button
                                onClick={() => formStep === 0 ? navigate("/video") : setFormStep(prev => prev - 1)}
